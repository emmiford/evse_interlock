...
{
	return telemetry_build_evse_payload_ex(buf, buf_len, device_id, device_type,
					       timestamp_ms, evt, false);
}

int telemetry_build_evse_payload_ex(char *buf, size_t buf_len, const char *device_id,
				    const char *device_type, int64_t timestamp_ms,
				    const struct evse_event *evt, bool time_anomaly)
{
	...
	int len = snprintf(
		...
		"\"timestamp\":%lld,\"time_anomaly\":%s,\"event_type\":\"%s\","
		...
		device_id, device_type, (long long)timestamp_ms,
		time_anomaly ? "true" : "false", evt->event_type,
